...
shinyServer(
  shinyServer(function(input, output, session) {
    ...
    one_row[1,]$family = '' 
    one_row[1,]$famsize = 0
    one_row[1,]$Mother = 'Not Mother' 
    ##C/Q/S
    one_row[1,]$Embarked = 'S'
    one_row[1,]$Fare = 80
    one_row[1,]$fsizeD = 'single'
    one_row[1,]$Child = 'Adult'
    
   
    
    
    output$ResultofSurvived <- renderText({ 
      
    one_row[1,]$Pclass = input$class
    one_row[1,]$Name = input$Name  
    one_row[1,]$Sex = input$plec
    one_row[1,]$Age = input$age
    one_row[1,]$title = input$title 
    one_row[1,]$surname = input$Second
    
    
    print(one_row[1,]$Fare)  ############## PRINT 
      ###
      if (input$class == "1")
      {
        one_row[1,]$Fare = 70
    ...
    print(input$age)
      ...
      {
        one_row[1,]$Child = 'Child'
        
      }
      #print(input$class)
      #print('FARE')
      #print(one_row[1,]$Fare)  
      ############## PRINT print(one_row)

      one_prediction <- predict(titanic_model, one_row)
      #print(variable)
      
      if (one_prediction == 1) 
      {
       
        paste("Śmiało wskauj na pokład!. Najprawdopodobniej przeżyjesz ",input$Name )
      }
      else if (one_prediction == 0) 
      {
        paste("Zginiesz!!! Kup bilet na inny statek!")
